...
//
use crate::{{leb128::*, WasmDecodeErrorKind, WasmMemArg, WasmBlockType}};
        ...
        match opcode.params.len() {
            ...
            1 => match opcode.params[0].as_str() {
                "br_table" => {
                    writeln!(
                        os,
                        "                let table_len: usize = reader.read()?;
                let table_len = table_len + 1;
                let mut table = Vec::with_capacity(table_len);
                for _ in 0..table_len {{
                    let br: u32 = reader.read()?;
                    table.push(br);
                }}
                Ok(Self::{}(table.into_boxed_slice()))",
                        opcode.identifier,
                    )
                    .unwrap();
                }
