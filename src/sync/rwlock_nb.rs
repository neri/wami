...
mod tests {
    ...
    #[test]
    fn rwlock() {
        let rwlock = RwLockNb::new(123);

        {
            let lock1 = rwlock.try_read().unwrap();
            rwlock.try_write().unwrap_err();
            let lock2 = rwlock.try_read().unwrap();
            rwlock.try_write().unwrap_err();
            let lock3 = rwlock.try_read().unwrap();
            rwlock.try_write().unwrap_err();
        ...
        {
            let mut lock1 = rwlock.try_write().unwrap();
            rwlock.try_write().unwrap_err();
            rwlock.try_read().unwrap_err();
            rwlock.try_write().unwrap_err();
